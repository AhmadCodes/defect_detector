...
import os
#%%
model_path = os.path.join(os.path.dirname(__file__), "../../weights/yolov8n/checkpoint.pt")
print(model_path)
model = ultralytics.YOLO(model_path)
# %%

def detect(image: np.ndarray,
           confidence_threshold: float = 0.2) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    for result in results:
        ...
        defect_image = cv2.rectangle(defect_image,
                                        (box[0], box[1]),
                                        (box[2], box[3]),
                                        color,
                                        2)
        # Overlay the bounding region on the defect map
        defect_map = cv2.rectangle(defect_map,
                                        (box[0], box[1]),
                                        (box[2], box[3]),
                                        (255,255,255),
                                        -1)
