...
def detect(image: np.ndarray,
           threshold: int = 20,
           defect_area_threshold: int = 50) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    _, binary_image = cv2.threshold(np.abs(gradient_y).astype(np.uint8),
    ...
    defects = []
    min_defect_area = defect_area_threshold  # Adjust this minimum area threshold as needed
