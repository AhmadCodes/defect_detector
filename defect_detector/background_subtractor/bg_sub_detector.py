...
def detect(image: np.ndarray, 
    ...
    gray_background = cv2.cvtColor(background, cv2.COLOR_BGR2GRAY) if len(background.shape) == 3 else background.copy()
    gray_current = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if len(image.shape) == 3 else image.copy()
...
if __name__ == "__main__":
    ...
    import time
    t0 = time.time()
    defect_image, defect_mask = detect(cv2.imread(test_image_path), cv2.imread(test_background_path))
