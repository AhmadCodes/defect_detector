...
def detect(image: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    _, threshold_image = cv2.threshold(
        image_, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )

    # apply morphological operations to remove noise
    kernel = np.ones((3, 3), np.uint8)
    threshold_image = cv2.morphologyEx(
        threshold_image, cv2.MORPH_OPEN, kernel, iterations=2
    )
    threshold_image = cv2.morphologyEx(
    ...
    contours, _ = cv2.findContours(
        defect_map, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    return contour_image, defect_map, bboxes
